...
#' @param pars list of length K vectors containing estimated prior probabilities, sample averages, and sample standard deviation
#' @return object containing `nK` matrix of posterior probabilities, and length `n` vector of predicted class
...
table(rownames(posterior),qda_pred)
```
...
The algorithm assumes that the relationship between input and response variables are characterised by simple linear regression, and computes the average TSME from k-fold cross validation of the regression.
